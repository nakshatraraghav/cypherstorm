...
func Verify(input, signaturePath string) error {
...
func loadPrivate(path string) (Private, error) {
	...
	if !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 {
		return Private{}, fmt.Errorf("identity: private identity must be a regular file")
	}
	data, err := boundedRead(path)
	if err != nil {
		return Private{}, err
...
func boundedRead(path string) ([]byte, error) {
	...
	defer f.Close()
	data, err := io.ReadAll(io.LimitReader(f, maxIdentitySize+1))
	if err != nil {
		return nil, err
	}
