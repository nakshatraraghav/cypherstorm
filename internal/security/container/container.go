// Package container implements CypherStorm's canonical envelope-encrypted protected-file format.
package container
...
func Encrypt(ctx context.Context, r io.Reader, w io.Writer, o EncryptOptions) error {
	...
	if o.Argon2 == (kdf.Argon2Params{}) {
		o.Argon2 = kdf.DefaultArgon2Params()
	}
	...
	defer wipe.Bytes(payloadKey)
	metadataKey, err := derive(contentKey, payloadID, "cypherstorm/v2/metadata", 32)
	if err != nil {
		return err
	}
	defer wipe.Bytes(metadataKey)
	stanzas, err := wrapRecipients(ctx, contentKey, payloadID, o.Recipients, o.Argon2)
	if err != nil {
		return err
	}
...
func Decrypt(ctx context.Context, r io.Reader, w io.Writer, o DecryptOptions) (compress.CompressionID, Metadata, error) {
	...
	contentKey, err := unwrapRecipients(ctx, header.Recipients, payloadID, o)
	if err != nil {
		return "", Metadata{}, err
	}
	defer wipe.Bytes(contentKey)
	...
	defer wipe.Bytes(metadataKey)
	metaAEAD, err := chacha20poly1305.NewX(metadataKey)
	if err != nil {
		return "", Metadata{}, err
	}
...
func Rekey(ctx context.Context, r io.Reader, w io.Writer, auth DecryptOptions, additions RecipientOptions, removeIDs []string, replaceSymmetric bool) (int64, error) {
	...
	payloadID, _ := base64.RawStdEncoding.DecodeString(header.PayloadID)
	contentKey, err := unwrapRecipients(ctx, header.Recipients, payloadID, auth)
	if err != nil {
		return 0, err
	}
	defer wipe.Bytes(contentKey)
	...
	if len(additions.Password) > 0 || len(additions.RawKey) > 0 || len(additions.PublicKeys) > 0 {
		added, wrapErr := wrapRecipients(ctx, contentKey, payloadID, additions, kdf.DefaultArgon2Params())
		if wrapErr != nil {
			return 0, wrapErr
		}
...
func readHeader(r io.Reader) (Header, int, []byte, error) {
	...
	if err := dec.Decode(&struct{}{}); err != io.EOF {
		return Header{}, 0, nil, fmt.Errorf("v2: header has trailing JSON data")
	}
	...
	if !bytes.Equal(data, canonical) {
		return Header{}, 0, nil, fmt.Errorf("v2: header is not canonical")
	}
	...
	passwordRecipients := 0
	recipientIDs := make(map[string]struct{}, len(h.Recipients))
	for _, stanza := range h.Recipients {
		switch stanza.Type {
		case "password":
			passwordRecipients++
			if passwordRecipients > 1 {
				return Header{}, 0, nil, fmt.Errorf("v2: multiple password recipients are not permitted")
			}
		case "raw-key":
		case "x25519":
			if stanza.ID == "" {
				return Header{}, 0, nil, fmt.Errorf("v2: x25519 recipient id is required")
			}
			if _, exists := recipientIDs[stanza.ID]; exists {
				return Header{}, 0, nil, fmt.Errorf("v2: duplicate recipient %s", stanza.ID)
			}
			recipientIDs[stanza.ID] = struct{}{}
		default:
			return Header{}, 0, nil, fmt.Errorf("v2: unknown recipient type %q", stanza.Type)
		}
	}
...
func wrapRecipients(ctx context.Context, contentKey, payloadID []byte, o RecipientOptions, argon2Params kdf.Argon2Params) ([]RecipientStanza, error) {
	...
	if len(o.Password) > 0 {
		...
		key := kdf.Argon2Params{Time: argon2Params.Time, MemoryKiB: argon2Params.MemoryKiB, Parallelism: argon2Params.Parallelism, KeyLength: 32}
		kek, err := kdf.DeriveMasterKey(ctx, kdf.Credential{Kind: kdf.SourcePassword, Password: o.Password}, key, salt)
		if err != nil {
			return nil, err
		}
		...
		wrapped := a.Seal(nil, nonce, contentKey, recipientAAD(payloadID, "password"))
		wipe.Bytes(kek)
	...
	if len(o.RawKey) > 0 {
		...
		a, _ := chacha20poly1305.NewX(kek)
		wrapped := a.Seal(nil, nonce, contentKey, recipientAAD(payloadID, "raw-key"))
		wipe.Bytes(kek)
...
func unwrapRecipients(ctx context.Context, stanzas []RecipientStanza, payloadID []byte, o DecryptOptions) ([]byte, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	for _, s := range stanzas {
		...
		case "password":
			...
			key, e = kdf.DeriveMasterKey(ctx, kdf.Credential{Kind: kdf.SourcePassword, Password: o.Password}, params, salt)
			if e != nil {
				return nil, e
			}
		case "raw-key":
		...
		plain, e := a.Open(nil, nonce, wrapped, recipientAAD(payloadID, s.Type))
...
func recipientAAD(payloadID []byte, recipientType string) []byte {
	aad := make([]byte, len(payloadID)+len(recipientType))
	copy(aad, payloadID)
	copy(aad[len(payloadID):], recipientType)
	return aad
}
