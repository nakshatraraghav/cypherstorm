...
func (s *KeychainStore) Put(ctx context.Context, name string, c SecretCredential, d Descriptor) error {
	...
	if err != nil {
		return fmt.Errorf("credential: encode keychain index: %w", err)
	}
	previous, previousErr := s.backend.Get(serviceName, name)
	if previousErr != nil && !errors.Is(previousErr, keyring.ErrNotFound) {
		return fmt.Errorf("credential: read existing OS keychain record: %w", previousErr)
	}
	...
	if err = s.backend.Set(serviceName, indexAccount, string(index)); err != nil {
		...
		if errors.Is(previousErr, keyring.ErrNotFound) {
			rollbackErr = s.backend.Delete(serviceName, name)
		} else {
			rollbackErr = s.backend.Set(serviceName, name, previous)
		}
		if rollbackErr != nil {
			return fmt.Errorf("credential: update keychain index: %w (credential rollback failed: %v)", err, rollbackErr)
		}
...
func (s *KeychainStore) Delete(ctx context.Context, name string) error {
	...
	index, err := json.Marshal(out)
	if err != nil {
		return fmt.Errorf("credential: encode keychain index: %w", err)
	}
	previous, err := s.backend.Get(serviceName, name)
	if err != nil {
		return fmt.Errorf("credential: read OS keychain record: %w", err)
	}
	if err = s.backend.Delete(serviceName, name); err != nil {
		return fmt.Errorf("credential: remove from OS keychain: %w", err)
	}
	if err = s.backend.Set(serviceName, indexAccount, string(index)); err != nil {
		if rollbackErr := s.backend.Set(serviceName, name, previous); rollbackErr != nil {
			return fmt.Errorf("credential: update keychain index: %w (credential rollback failed: %v)", err, rollbackErr)
		}
		return fmt.Errorf("credential: update keychain index: %w", err)
	}
